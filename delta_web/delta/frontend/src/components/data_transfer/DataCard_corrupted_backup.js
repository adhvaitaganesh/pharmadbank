...
const DataCard = ({ data, style, isSelected, onCheckboxChange }) => {
    ...
    return ( <
        ...
        <
        strong > Tags: < /strong> <
        div className = "mt-1" > { data.tags.map((tag, index) => ( < div className = { styles.tag_item }
                key = { index } > < span className = { styles.text } > { tag.text } < /span></div > )) } < /div> <
        /div> <
        /div> {
            data.files && data.files.length > 0 && ( <
                ...
                div className = { fileStyles.filesList } > {
                    data.files.map((file, index) => ( <
                        ...
                        span className = { `${fileStyles.fileExt} ${getFileTypeBadgeClass(file.file_type)}` } > { file.file_type || 'unknown' } < /span> <
                        /div>
                    ))
                } <
                /div> <
        ...
        div className = "position-absolute bottom-0 end-0 mb-2 me-2 d-flex align-items-center" > < FaArrowDown className = "text-muted me-2"
        style = {
            { fontSize: '1.5rem' } }
        /><span className="badge bg-secondary">{data.download_count}</span > < /div> <
        /div> <
        /div> <
        /span>
    );
