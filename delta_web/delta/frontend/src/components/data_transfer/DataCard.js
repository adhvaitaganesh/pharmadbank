...
const DataCard = ({ data, style, isSelected = false, onCheckboxChange = null }) => {
    ...
    const renderStars = (rating) => { const stars = []; if (rating === 0 || rating === -1) { stars.push(React.createElement(FaStar, { key: 0, color: "gray", className: "mr-1" })); return stars; } for (let i = 0; i < 5; i++) stars.push(React.createElement(FaStar, { key: i, color: i <= rating ? "gold" : "gray", className: "mr-1" })); return stars; };
    const descriptionStyle = { height: "6rem", overflowY: "auto", paddingRight: "1rem" };
